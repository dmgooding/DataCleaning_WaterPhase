...
for ntuple in input_file_list: #loop over files in a directory
	...
	PrelimMask = int(239693534871552)#different prelim mask!
	...
	for i in PassIndices: #This one is okay
		...
		if data.energy[i] <= E_LOW:
			continue
		rat_r = (data.posr[i]**3.)/(6000.**3.)
		if rat_r >= POSR_CUT_HIGH:
			continue
		if rat_r <= POSR_CUT_LOW:
			continue
		#if data.posz[i] < Z_LOW:
		#	continue
		#if data.posz[i] > Z_HIGH:
		#	continue
		if data.nhits[i] >= nhitsCleaned_HIGH:
			continue
		...
		udotr = u/n
		corr = 12.0*((data.posr[i]**3.0/6000.0**3) - 0.79)**2
		#if udotr < udotr_LOW - corr:
		#	continue
		if udotr >= udotr_HIGH:
			continue
	...
	for j in PrelimpassIndices: #This needs to be histogram of everythign that passed prelim ****put iscal here****
		...
		if data.fitValid[j] == False:
			failed_fit.append(j)
			continue
		if data.waterFit[j] == 0:
			failed_water.append(j)
			continue
		...
		#	continue
		if data.nhits[j] >= nhitsCleaned_HIGH:
			continue
		...
		udotr = u/n
		corr = 12.0*((data.posr[j]**3.0/6000.0**3) - 0.79)**2
		#if udotr < udotr_LOW - corr:
		#	continue
		if udotr >= udotr_HIGH:
			failed_ur.append(j)
	...
	for index in PassClassIndices:
		...
		if data.energy[index] <= E_LOW:
			continue
		rat_r = (data.posr[index]**3.)/(6000.**3.)
		if rat_r >= POSR_CUT_HIGH:
			continue
		if rat_r <= POSR_CUT_LOW:
			continue
		...
		#	continue
		if data.nhits[index] >= nhitsCleaned_HIGH:
			continue
		...
		udotr = u/n
		corr = 12.0*((data.posr[index]**3.0/6000.0**3) - 0.79)**2
		#if udotr < udotr_LOW - corr:
		#	continue
		if udotr >= udotr_HIGH:
			continue

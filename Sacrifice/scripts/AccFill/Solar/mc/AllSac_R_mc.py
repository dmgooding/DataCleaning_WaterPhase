...
for ntuple in input_file_list: #loop over files in a directory
	...
	#test DCMask = int(36283883798526)
	DCMask = int(36283883798270)
	#original logic from Brian for physics trigger:
	# if (!(triggerWord & 0x3F) || (triggerWord & 0xBEF9400))) continue
...
#	TotalDCMask = PrelimMask + DCMask
	...
	E_HIGH_1 = 5.0 #mev
	E_LOW_1 = 3.5 #mev
	POSR_CUT_HIGH_1 = 4400.0 #mm
	E_HIGH_2 = 15.0 #Energy-dependent FV cuts follow
	E_LOW_2 = 5.0
	POSR_CUT_HIGH_2 = 5300.0 #mm
#	Z_LOW = -6000.0 #mm
#	Z_HIGH = 6000.0 #mm
#	Z_HIGH = 1500.0 #mm changed on 3/16/21 on morgan's update
	nhitsCleaned_LOW = 15
...
#	sunct_LOW = -0.7
	...
	for i in PassIndices: #This one is okay
		...
		if data.isCal[i] == False:
			continue
		if data.runID[i] >= 104613 and data.inTimeHits100[i] < IN_TIME_HITS100:
			continue
		if data.runID[i] < 104613 and data.inTimeHits100[i] < IN_TIME_HITS100_early:
			continue
	...
	#	if data.itr[i] <= ITR:
	#		continue
		if data.energy[i] > E_LOW_1 and data.energy[i] < E_HIGH_1:
			if data.posr[i] > POSR_CUT_HIGH_1:
				continue
		if data.energy[i] > E_LOW_2 and data.energy[i] < E_HIGH_2:
			if data.posr[i] > POSR_CUT_HIGH_2:
				continue
		if data.energy[i] <= E_LOW_1 or data.energy[i] >= E_HIGH_2:
			continue
...
#			continue
	...
	for j in PrelimpassIndices: #This needs to be histogram of everythign that passed prelim ****put iscal here****
		...
		if data.isCal[j] == False:
			continue
		if data.runID[j] >= 104613 and data.inTimeHits100[j] < IN_TIME_HITS100:
			continue
		if data.runID[j] < 104613 and data.inTimeHits100[j] < IN_TIME_HITS100_early:
			continue
	...
	#	if data.itr[j] <= ITR:
	#		continue
		if data.energy[j] > E_LOW_1 and data.energy[j] < E_HIGH_1:
			if data.posr[j] > POSR_CUT_HIGH_1:
				continue
		if data.energy[j] > E_LOW_2 and data.energy[j] < E_HIGH_2:
			if data.posr[j] > POSR_CUT_HIGH_2:
				continue
		if data.energy[j] <= E_LOW_1 or data.energy[j] >= E_HIGH_2:
			continue
...
#			continue
	...
	for index in PassClassIndices:
		...
		if data.isCal[index] == False:
			continue
		if data.runID[index] >= 104613 and data.inTimeHits100[index] < IN_TIME_HITS100:
			continue
		if data.runID[index] < 104613 and data.inTimeHits100[index] < IN_TIME_HITS100_early:
			continue
		#if data.inTimeHits100[index] < IN_TIME_HITS100:
		#	continue
	...
	#		continue
		if data.energy[index] > E_LOW_1 and data.energy[index] < E_HIGH_1:
			if data.posr[index] > POSR_CUT_HIGH_1:
				continue
		if data.energy[index] > E_LOW_2 and data.energy[index] < E_HIGH_2:
			if data.posr[index] > POSR_CUT_HIGH_2:
				continue
		if data.energy[index] <= E_LOW_1 or data.energy[index] >= E_HIGH_2:
			continue
